        ...
        <div className="px-3 mb-3 flex-shrink-0">
          <div className="bg-gray-800 rounded-lg p-2.5 border border-gray-700 flex items-center gap-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-white font-medium text-xs truncate">{user.name} {user.lastName}</p>
              <p className={`text-[10px] font-medium ${roleInfo.color}`}>{roleInfo.label}</p>
              {user.isVerified === false && <p className="text-red-400 text-[10px]">No verificado</p>}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="px-3 py-3 flex-shrink-0">
        <div className="bg-gray-800 rounded-lg p-3">
          ...
          <div className="bg-gray-600 rounded-full h-1.5 mb-2">
            <div className={`${progressInfo.color} h-1.5 rounded-full transition-all duration-500 ease-in-out`} style={{ width: `${progress}%` }} />
          </div>
