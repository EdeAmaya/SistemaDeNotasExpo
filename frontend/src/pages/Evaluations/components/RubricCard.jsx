...
import { ClipboardList, Award, BookOpen, Calendar, Edit2, Trash2, FileText, CheckCircle, Layers, GraduationCap, Target } from 'lucide-react';

const RubricCard = ({ rubric, deleteRubric, updateRubric, viewMode = 'list' }) => {
  ...
  const getRubricTypeColor = (type) => {
    // Colores basados en el tipo numérico
    if (type === 1) return 'bg-purple-600'; // Escala Estimativa
    if (type === 2) return 'bg-purple-600'; // Rúbrica
    return 'from-gray-500 via-gray-600 to-gray-700'; // Default (gris)
  };

  const getRubricTypeIcon = (type) => {
    if (type === 1) return <Target className="w-3 h-3" />; // Escala Estimativa
    if (type === 2) return <CheckCircle className="w-3 h-3" />; // Rúbrica
  ...
  return (
    <div className="group relative bg-white rounded-xl shadow-sm hover:shadow-xl border-2 border-gray-100 hover:border-gray-200 transition-all duration-300 overflow-hidden">
      ...
      <div className="p-6 space-y-4">
        ...
        {/* Criterios */}
        <div className={`flex items-center gap-2 px-4 py-2.5 rounded-lg ${getCriteriaCount() > 0
