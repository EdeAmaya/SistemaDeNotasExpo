          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-4 text-sm flex-wrap">
              <div className="flex items-center gap-1.5 text-gray-600">
                ...
                <span className="font-medium">{getLevelName(project.idLevel)}</span>
              </div>
              <div className="flex items-center gap-1.5 text-gray-600">
                <FileText className="w-4 h-4" />
                <span className="font-medium">{getSectionName(project.idSection)}</span>
              </div>
              {project.googleSitesLink && (
                <div className="flex items-center gap-1.5 text-purple-600">
                  <Globe className="w-4 h-4" />
                  <span className="font-medium text-xs">Sitio Web</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex-shrink-0 flex items-center gap-2">
            {project.googleSitesLink && (
              <button
                ...
                className="px-4 py-2 bg-purple-500 hover:bg-purple-600 text-white rounded-lg font-semibold text-sm shadow-sm hover:shadow-md transition-all duration-200"
                title="Ver sitio web"
              >
      ...
      <div className={`relative ${
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <h3 className="text-xl font-black mb-1">{project.projectName}</h3>
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex items-center gap-2 text-gray-700">
          ...
          <div>
            ...
            <div className="text-sm font-bold">{getLevelName(project.idLevel)}</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 text-gray-700">
          ...
          <div>
            ...
            <div className="text-sm font-bold">{getSectionName(project.idSection)}</div>
          </div>
        </div>

        {/* Sitio Web */}
        {project.googleSitesLink ? (
          <div className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-purple-50 border-2 border-purple-200">
            <Globe className="w-5 h-5 text-purple-600" />
            <div className="flex-1">
              <div className="text-xs text-purple-600 font-semibold">Sitio Web</div>
              <div className="text-sm font-bold text-purple-800">Disponible</div>
            </div>
          </div>
        ) : (
          <div className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-gray-50 border-2 border-gray-200">
            <Globe className="w-5 h-5 text-gray-400" />
            <div className="flex-1">
              <div className="text-xs text-gray-500 font-semibold">Sitio Web</div>
              <div className="text-sm font-bold text-gray-600">Sin enlace</div>
            </div>
          </div>
        )}
        ...
        <div className="space-y-2 pt-2">
          {project.googleSitesLink && (
            <button
              onClick={openGoogleSite}
              className="w-full bg-purple-500 hover:bg-purple-600 text-white py-2.5 rounded-lg font-bold text-sm shadow-md hover:shadow-lg transition-all duration-200 flex items-center justify-center gap-2"
            >
              <ExternalLink className="w-4 h-4" />
